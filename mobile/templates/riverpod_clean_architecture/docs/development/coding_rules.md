...
### Infrastructure Gatewayの単位

Gateway実装は外部機能・保存基盤ごとの独立したローカルpackageにする。たとえば
`local_database_gateway`、`photo_library_service`、`location_name_gateway`のように、依存する
外部システムと責務がpackage名から判断できる単位とする。複数の無関係なGateway実装を
一つの共通`gateway` packageへ収容しない。

同じDatabase、同じローカル保存領域、同じ外部永続化サービスへ接続する処理は、
Infrastructure側で一つのGateway実装にまとめる。AggregateやUsecase interfaceが異なることだけを
理由に、接続、transaction管理、DTO mapper、error mappingを持つ実装クラスを分割しない。

Usecase側のGateway interfaceは呼び出し側の責務に合わせて分けてよい。一つのInfrastructure
Gatewayが複数のinterfaceを実装し、同じ接続とtransaction境界を共有する。

```text
StoneGateway ─┐
WishGateway  ─┴─ local_database_gateway package ─ SQLite
```

Gateway実装の分割を検討するのは、保存先のDatabaseや外部サービス、接続設定、transaction境界、
ライフサイクルのいずれかが異なる場合とする。単にファイルが長くなった場合は、Gateway自体を
分けるのではなく、DTO mapperや操作実装ファイルを責務ごとに分割して同じGatewayから利用する。
...
外部package・libraryは完全なバージョンを指定する。`^1.2.3`、`~1.2.3`、
`>=1.2.3 <2.0.0`のように、依存解決時に別バージョンを自動選択できる指定は使わない。
...
新しいリリースが汚染されていた場合に、通常の依存解決だけでそのバージョンを取り込む
supply chain riskを避けるためである。依存を更新するときはリリース内容と安全性を確認し、
指定バージョンを明示的に変更してformat、analyze、testを実行する。

Flutter SDKなど、package managerではなく専用のversion managerで管理するものも、プロジェクトの
設定ファイルで利用バージョンを固定する。リポジトリ内のpath dependencyは、参照先を同じ
リポジトリで管理するためこの規則の対象外とする。
